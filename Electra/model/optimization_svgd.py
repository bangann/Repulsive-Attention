...
def create_optimizer(
        ...
        layerwise_lr_decay_power=-1, n_transformer_layers=None, num_attention_heads=4, d_kernel_weight=1, stepsize=1):
    ...
    # svgd
    grads, tvars = svgd(zip(grads, tvars), num_attention_heads, d_kernel_weight, stepsize)
...
def svgd(grads_vars, num_attention_heads, d_kernel_weight, stepsize):
    grads = []
    vars = []
    for grad, var in grads_vars:
        if ("electra" in var.name) and ("attention/self" in var.name) and ("kernel" in var.name) and ("layer_0" in var.name):
            print(var.name)
            grad = svgd_grad(grad, var, num_attention_heads, d_kernel_weight, stepsize)
        grads.append(grad)
        vars.append(var)
    return grads, vars


def svgd_grad(grad, var, num_attention_heads, d_kernel_weight, stepsize):
    hidden_size = grad.get_shape()[0]
    size_per_head = grad.get_shape()[1] // num_attention_heads
    grad_reshape = tf.reshape(grad, [hidden_size * size_per_head, num_attention_heads])
    var_reshape = tf.reshape(var, [hidden_size * size_per_head, num_attention_heads])
    grad_reshape = tf.transpose(grad_reshape, [1, 0])
    var_reshape = tf.transpose(var_reshape, [1, 0])

    Kxy, dxkxy = svgd_kernel(var_reshape, num_attention_heads)
    grad_svgd = stepsize * (tf.matmul(Kxy, grad_reshape) - d_kernel_weight * dxkxy) / num_attention_heads
    grad_svgd = tf.transpose(grad_svgd, [1, 0])
    grad_svgd = tf.reshape(grad_svgd, [hidden_size, num_attention_heads * size_per_head])
    return grad_svgd


def svgd_kernel(stacked_vars, num_attention_heads, median_heuristic=True):
    ...
    if median_heuristic:
        ...
        h = tf.sqrt(0.5 * median / tf.log(num_attention_heads + 1.))
