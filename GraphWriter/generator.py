...
# from models.newmodel_2 import model
...
def test(args,ds,m,epoch='cmdline'):
  ...
  # ofn = "../outputs/" + model + ".beam_predictions" + "multi"
  m.eval()
  k = 0
  data = ds.mktestset(args)
  ofn = "outputs/"+model+".inputs.beam_predictions."+epoch
  # ofn = "outputs/" + model + ".inputs.beam_predictions." + epoch + "multi"
  pf = open(ofn,'w')
  ofn_gold = "outputs/"+model+".inputs.gold."+epoch
  # ofn_gold = "outputs/" + model + ".inputs.gold." + epoch + "multi"
  pf = open(ofn,'w')
  gd = open(ofn_gold, 'w')
  preds = []
  golds = []
  for b in data:
    ...
    # # gen.sort()
    # for i in range(4):
    #   gen_i = ds.reverse(gen.done[i].words, b.rawent)
    #   print(gen_i)
    #   print()
    #   preds.append(gen_i.lower())
    #   pf.write(gen_i.lower() + '\n')
    # k += 1
