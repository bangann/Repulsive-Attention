...
class SelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.ws2 = Linear_SVGD.apply
        self.Weight2 = nn.Parameter(nn.init.uniform(torch.Tensor(config.attention_hops, config.attention_unit), -0.1, 0.1), requires_grad=True)
    ...
    def forward(self, inp, outp):
        ...
        alphas = self.ws2(hbar, self.Weight2, self.config).view(size[0], size[1], -1)  # [bsz, len, hop]
...
class Linear_SVGD(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.config.bayesian_method != 'None':
            if ctx.config.bayesian_method == 'svgd':
                grad_weight = svgd(weight, grad_weight, ctx.config.alpha)
            elif ctx.config.bayesian_method == 'spos':
                grad_weight = spos(weight, grad_weight, ctx.config.alpha, ctx.config.beta, ctx.config.stepsize)
            grad_weight = grad_weight * ctx.config.stepsize

        grad_weight = grad_weight.float().cuda()
